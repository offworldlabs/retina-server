...
export interface MapPalette {
  ...
  DRONE: string;
  /** A drone's detection arc: lighter than its glyph, so the arc under a
   *  drone reads as its arc rather than as a second drone. */
  DRONE_ARC: string;
  /** The beam-gap diagnostic line. Deliberately not ANOMALY: the two reds
   *  have to stay apart where a gap line crosses an anomalous track. */
  BEAM_GAP: string;
...
let _activeTheme: MapTheme = DEFAULT_MAP_THEME;

export function setActivePalette(p: MapPalette, theme: MapTheme): void {
  _active = p;
  _activeTheme = theme;
}
...
/** Which theme is drawn, for the rare consumer that needs more than a colour —
 *  a basemap URL, or a surface that mounts outside the token scope. */
export function activeTheme(): MapTheme {
  return _activeTheme;
}
