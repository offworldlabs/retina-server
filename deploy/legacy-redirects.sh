...
# repo answers on them: they are Cloudflare redirect rules into the equivalent
# path on their environment's `app` host, and if a rule is removed or misedited
...
# assert_legacy_redirect <retired-host> <expected-target-prefix>
#
# e.g. assert_legacy_redirect dash.retina.fm https://app.retina.fm/dash
#
...
assert_legacy_redirect() {
    local host="$1" prefix="$2" curl_opts probe result code loc want
    curl_opts="-s --connect-timeout 10 --max-time 30"

    # A path and a query string, so this also asserts the rule carries both
    # through. A rule written with `preserve_query_string` off and a static
    # target drops them, which for a bookmarked deep link is a silent landing on
    # the wrong page rather than an error.
    probe="/smoke-redirect?keep=1"
    want="${prefix}${probe}"
