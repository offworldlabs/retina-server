...
def test_immutable_is_confined_to_hashed_assets(rendered):
    """Only a Vite `assets/` tree may be kept: a content-hashed name is the
    property that makes the week safe. The console's theme-boot.js is the name
    this guards: no hash, and loaded by every page before its first paint."""
