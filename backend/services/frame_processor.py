...
def process_one_frame(node_id: str, frame: dict, default_pipeline: PassiveRadarPipeline):
    ...
    # indexes these arrays, and the known lane below renumbers them.  Fail open,
    # as the known lane does: a mirrored frame reaches here untyped, and a
    # malformed track must not cost the frame its detections.
    if "tracker" in frame:
        try:
