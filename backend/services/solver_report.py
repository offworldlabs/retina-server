...
def _median_p90(values: Iterable[float]) -> tuple[float | None, float | None]:
    """Median and p90 of ``values``, both None when it is empty.

    Nearest rank with no interpolation: the median is the upper-middle
    element and the p90 the one at int(0.9 * (n - 1)).  This is not numpy's
    percentile, which analytics_refresh uses, and the two disagree on
    even-sized samples.
    """
    xs = sorted(values)
    n = len(xs)
    if not n:
        return None, None
    return xs[n // 2], xs[int(0.9 * (n - 1))]


# ── Solver Report payload blocks, in payload order ────────────────────────────


def _split_lanes(records: list[dict]) -> tuple[dict[str, int], list[dict], list[dict]]:
    """Classify one window's records by ``_record_lane``.

    Returns the per-lane counts published as ``lane_split``, then the dark and
    known records the lane-specific blocks are computed from.  ADS-B and
    dark-follow records are only counted.
    """
    lane_split = {"dark": 0, "adsb": 0, "known": 0, "dark_follow": 0}
    dark_records: list[dict] = []
    known_records: list[dict] = []
    for r in records:
        lane = _record_lane(r)
        lane_split[lane] = lane_split.get(lane, 0) + 1
        if lane == "dark":
            dark_records.append(r)
        elif lane == "known":
            known_records.append(r)
    return lane_split, dark_records, known_records


def _dark_funnel(dark_records: list[dict]) -> dict:
    """The top-level ``attempts``, ``published``, ``rejects`` and
    ``position_error_km``: the dark lane's publication funnel (see
    _solver_window_stats for why it is dark-only).

    A reject is any outcome other than ``published``, keyed by its reason with
    the ``rejected_`` prefix stripped.  Position error is taken over published
    solves whose gt_error_km is within _ERR_GT_GATE_KM.
    """
    n2 = n3plus = 0
    reject_total = 0
    ...
    median, p90 = _median_p90(pos_errors)
    return {
        "attempts": len(dark_records),
        "published": {"total": n2 + n3plus, "n2": n2, "n3plus": n3plus},
        "rejects": {"total": reject_total, "by_reason": by_reason},
        "position_error_km": {"median": median, "p90": p90, "n": len(pos_errors)},
    }


def _contamination(dark_records: list[dict]) -> dict:
    """Cluster contamination, dark and windowed.
    ...
    """
    judged = [r for r in dark_records if r.get("foreign_node_ids") is not None]
...
def _resolve_skips(cutoff_ms: int, minutes: float, dark_attempts: int) -> dict:
    """Resolve-slot skips inside the window, from the skip deque.

    ``counters.resolve_skips`` is since boot; these are the skips that
    ...
    return {
        ...
        "attempts_ratio": round(len(skips) / dark_attempts, 3) if dark_attempts else None,
        ...
        # cover it.  Same honesty rule as the payload's top-level
        # window_effective_minutes: read it before reading total as a window
        # count.
        "window_effective_minutes": _window_effective_minutes(all_skips, minutes),
    }


def _live_ghosts() -> dict:
    """Point-in-time ghost scan of state.multinode_tracks, DARK tracks only.
...
def _ghosts(published_records: list[dict]) -> dict:
    """The ``ghosts`` block, windowed over the funnel's published dark records
    (see _solver_window_stats's docstring); precision_pct is None (not 100.0)
    when nothing in the window could be judged.  ``live`` is the point-in-time
    scan of state.multinode_tracks the block used to consist of.
    """
    return {
        "scope": "dark",
        "gate_km": _GHOST_GATE_KM,
        **_windowed_ghosts(published_records),
        "live": _live_ghosts(),
    }


def _consensus() -> dict:
    """Consensus counters, since boot and cumulative regardless of mode: the
    convention the claiming, fov and known_lane blocks follow."""
    return {
...
def _claiming() -> dict:
    """Top-down claiming, since boot — same "cumulative regardless of mode"
    convention as consensus.  rounds/matched/conflicts/anchored_inputs/
    tracklets_excluded come off the library associator (the claiming stage
    itself); anchor_hits/anchor_fallbacks/anchored_published are the
    solver-side honoring outcome.
    """
    return {
...
def _fov() -> dict:
    ...
    "cumulative regardless of mode" convention as claiming/consensus.  See
    solver.py's beam gate for what agree/would_pass/would_reject mean.
    """
    return {
        "mode": state.FOV_MODE,
...
def _counter_snapshot() -> SimpleNamespace:
    ...
    with state.counters_lock:
        return SimpleNamespace(
            known_lane_attempts=state.known_lane_attempts,
            known_lane_truth_match=state.known_lane_truth_match,
            known_lane_ghost=state.known_lane_ghost,
            known_lane_no_converge=state.known_lane_no_converge,
            known_lane_published=state.known_lane_published,
            ...
            known_claims_visibility_rejects=state.known_claims_visibility_rejects,
            known_claims_world_rejects=state.known_claims_world_rejects,
            known_claims_errors=state.known_claims_errors,
            known_hold_claims=state.known_hold_claims,
            known_hold_expired=state.known_hold_expired,
            known_hold_dropped_disagree=state.known_hold_dropped_disagree,
            known_follow_claims=state.known_follow_claims,
            calibration_points_recorded=state.calibration_points_recorded,
            ...
            dark_follow_kept_manoeuvre=state.dark_follow_kept_manoeuvre,
            dark_follow_gate_sigma_clamped=state.dark_follow_gate_sigma_clamped,
            dark_follow_inelig={
...
def _known_lane_position_error(known_records: list[dict], minutes: float) -> dict:
    """Known-lane position error, windowed: the lane's own error metric, so its
    accuracy is readable now that its records no longer pass through the dark
    funnel.

    displacement_km, not gt_error_km: the known lane's initial guess IS the
    ADS-B fix dead-reckoned to the solve epoch, so displacement from it is
    exactly the solver-vs-truth error the lane exists to measure
    (known_lane._attempt says the same), and unlike gt_error_km it is
    populated against real traffic where no synthetic trail exists.  Ghosts
    are deliberately included — a ghost's error is the datum the regular
    pipeline's displacement gate deletes.
    """
    errors = [r["displacement_km"] for r in known_records if r.get("displacement_km") is not None]
    median, p90 = _median_p90(errors)
    return {"median": median, "p90": p90, "n": len(errors), "window_minutes": minutes}


def _known_lane(snap: SimpleNamespace, known_records: list[dict], minutes: float) -> dict:
    ...
    attempts is every claimed hex the lane tried; truth_match / ghost /
    no_converge partition those attempts by outcome (see
    ...
    reach the feed — it accounts for exactly the truth_match minus published
    discrepancy that otherwise only the logs can explain.
    """
    return {
        "mode": state.KNOWN_LANE_MODE,
        "attempts": snap.known_lane_attempts,
        "truth_match": snap.known_lane_truth_match,
        "ghost": snap.known_lane_ghost,
        "no_converge": snap.known_lane_no_converge,
        "published": snap.known_lane_published,
        "publish_errors": snap.known_lane_publish_errors,
        ...
        "reanchored": snap.known_lane_reanchored,
        ...
        # see _known_lane_position_error.
        "position_error_km": _known_lane_position_error(known_records, minutes),
    }


def _dark_follow(snap: SimpleNamespace) -> dict:
    """Dark track following (services/dark_follow.py), since boot except
    targets_now, a live gauge of the current pseudo-state list.  The funnel
    repeats six of the ``counters`` block's entries because it is only
    readable beside "ineligible", which is the other half of the same walk:
    every dark key in state.multinode_tracks is either a target or one of
    those reasons, re-tested on every rebuild.

    ineligible IS IN KEY-SECONDS, the funnel is in events.  The target list is
    rebuilt once a second and every dark key is re-tested, so a key that is
    ineligible for a minute adds ~60 to its reason.  Compare the reasons with
    each other (which gate holds the lane back) and with targets_now; do NOT
    divide them by inputs or published.
    """
    return {
        "mode": dark_follow.mode(),
        "targets_now": snap.dark_follow_targets,
        "inputs": snap.dark_follow_inputs,
        "published": snap.dark_follow_published,
        "dropped": snap.dark_follow_dropped,
        ...
        "n2_withheld": snap.dark_follow_n2_withheld,
        "n2_skipped": snap.dark_follow_n2_skipped,
        ...
        "kept_manoeuvre": snap.dark_follow_kept_manoeuvre,
        "gate_sigma_clamped": snap.dark_follow_gate_sigma_clamped,
        "ineligible": snap.dark_follow_inelig,
    }


def _known_claims(snap: SimpleNamespace) -> dict:
    """Claiming stage feeding the known lane (services/known_claiming.py), also
    since boot.  made counts every claim recorded in either mode; bound the
    detections binding actually removed from the dark pool, so made > 0 with
    bound == 0 is the shadow-soak signature.  errors nonzero means the
    claiming stage is throwing and the lane is silently contributing nothing.
    """
    return {
        "made": snap.known_claims_made,
        "contentions": snap.known_claim_contentions,
        "bound": snap.known_claims_bound,
        "visibility_rejects": snap.known_claims_visibility_rejects,
        "world_rejects": snap.known_claims_world_rejects,
        "errors": snap.known_claims_errors,
        ...
        "hold_claims": snap.known_hold_claims,
        "hold_expired": snap.known_hold_expired,
        "hold_dropped_disagree": snap.known_hold_dropped_disagree,
        ...
        "follow_claims": snap.known_follow_claims,
        ...
        "calibration_recorded": snap.calibration_points_recorded,
        "calibration_rejected": snap.calibration_claims_rejected,
    }


def _pool(published_records: list[dict]) -> dict:
    ...
    is how much of the window the number actually speaks for.  pct is null
    rather than 0 when nothing in the window carried the stamp, because "no
    solve was narrower than its pool" and "no solve was measured" are not the
    same answer.
    """
...
def _fragmentation(published_records: list[dict]) -> dict:
    """Windowed, from the same published records the funnel counts —
    distinct published keys is the acceptance metric top-down claiming
    exists to move: O(targets) instead of O(solves).  anchored_pct is the
    share of those published solves whose input carried an anchor_key.
    Rejects carry one too (solve_history._record_solve_history stamps every
    record) but are not counted here.
    """
    ...
    spk_median, spk_p90 = _median_p90(key_counts.values())
    return {
...
def _solver_window_stats(minutes: float) -> dict:
    ...
    lane_split, dark_records, known_records = _split_lanes([r for r in merged if r["ts_ms"] >= cutoff_ms])
    published_records = [r for r in dark_records if r.get("outcome") == "published"]
    ...
    return {
        ...
        "window_effective_minutes": _window_effective_minutes(merged, minutes),
        # Denominator for the dark-lane funnel below: how many records in this
        # window each lane wrote.  Sums to the whole merged window, so the
        # funnel can be read against what it excludes.
        "lane_split": lane_split,
        # ── DARK LANE ONLY, from here to fragmentation ──────────────────────
        # attempts, published, rejects and position_error_km.
        **_dark_funnel(dark_records),
        ...
        "by_n_nodes": _by_n_nodes(dark_records),
        ...
        "contamination": _contamination(dark_records),
        "resolve_skips": _resolve_skips(cutoff_ms, minutes, len(dark_records)),
        "ghosts": _ghosts(published_records),
        "consensus": _consensus(),
        "claiming": _claiming(),
        "fov": _fov(),
        "known_lane": _known_lane(snap, known_records, minutes),
        "dark_follow": _dark_follow(snap),
        "known_claims": _known_claims(snap),
        "pool": _pool(published_records),
        "fragmentation": _fragmentation(published_records),
