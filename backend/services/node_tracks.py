...
class NodeTrackStore:
    ...
    def ingest(self, node_id: str, frame: dict) -> int:
        ...
        its arrays, and the known lane's strip renumbers them.  A frame naming
        no tracker run files nothing, but it still ages the node's tracks, and a
        node left holding none is forgotten: a node whose tracker stops does not
        keep its last tracks.  A frame whose `tracks` is absent is a tracker
        with no output for that frame.  A malformed track, possible only on a
        frame that arrived untyped, is refused alone: it changes nothing, and
        the frame's other tracks are filed regardless.
        """
